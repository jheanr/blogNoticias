        ...
        public static async Task CreateUser(PetNewsApiApplication application, bool create)
        {
            ...
            using (var scope = application.Services.CreateScope())
            {
                ...
                using (var petNewsDbContext = provider.GetRequiredService<BlogPetNewsDbContext>())
                {
                    ...
                    if (create)
                    {
                        var password = Cryptography.Encodes("gato@123");
                        
                        await petNewsDbContext.Users.AddAsync(new User { Name = "El Gato", Email = "elgato@miau.net", Password = password });
                ...
                using (var petNewsDbContext = provider.GetRequiredService<BlogPetNewsDbContext>())
                {
                    ...
                    if (create)
                    {
                        await petNewsDbContext.News.AddAsync(new News
                        {
                            ...
                            UserId = new Guid("AAA791FB-C736-49AD-8AE8-0AF96DD4F149")
                        });
